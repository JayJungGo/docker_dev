...
class CircuitBuilder(Protocol):
    ...
    ExperimentConfig에 정의된 code distance, rounds, noise parameter 등의 조건을
    이용하여 circuit 객체를 생성한다.
    유지보수 규칙:
...
class DetectorErrorModelBuilder(Protocol):
    ...
    이 기능을 CircuitBuilder와 분리한 이유는 회로 생성과 오류 모델 생성의
    책임을 독립적으로 관리하기 위함이다.
    """
...
class SyndromeSampler(Protocol):
    ...
    현재 pipeline에서 syndrome 데이터는 주로 detection event와 logical observable로
    구성한다. 이 객체는 decoding 및 성능 평가는 수행하지 않는다. 데이터 생성만 담당한다.
    """
    ...
    def sample(
        ...
        Returns:
          SampleBatch:
            detections: detector event 배열.
            observables: logical observable flip 배열.
        """
        ...
...
class Decoder(Protocol):
    ...
    ExperimentRunner는 구체적인 decoder 종류를 알 필요가 없으며,
    이 인터페이스의 decode()만 호출한다.
    """
    ...
    def decode(
        ...
        Args:
          ...
          detections:
            shape 예: (shots, num_detectors)
            각 shot에서 발생한 detection event 정보.
        Returns:
          np.ndarray:
            shape 예: (shots, num_observables)
...
class CircuitInspector(Protocol):
    ...
    Circuit 생성과 분석을 분리하여 CircuitBuilder가 지나치게 많은 책임을
    갖지 않도록 한다.
    """
    ...
    def get_detector_coordinates(
        ...
        이 정보는 향후 GNN dataset 생성 시 detector를 graph node로
        변환하는 데 활용할 수 있다.
        """
        ...
...
class RuntimeInfoProvider(Protocol):
    ...
    Python version, Stim version, PyMatching version, NumPy version 등
    시스템 환경을 metadata 생성 로직과 분리하기 위한 인터페이스다.
    """
...
class ResultWriter(Protocol):
    ...
    현재 구현: FileSystemResultWriter
    저장 방식이 바뀌더라도 simulation pipeline은 영향을 받지 않도록 한다.
    """
